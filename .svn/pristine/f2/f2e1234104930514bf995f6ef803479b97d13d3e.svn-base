...
<table style="width: 800px;">
    ...
    <tr>
        ...
        <g:each in="${eventInstance.phase.sort {it.orderPhase}}" var="eventPhase">
            <th title="${eventPhase.phase}" style="text-align: center" >
                <g:message code="event.phase.label"/> ${p}
                <sec:ifAnyGranted roles="ROLE_SUPERADMIN,ROLE_ADMIN">
                    <g:link controller="event" action="completeNewPhase" title="Completar etapas" id="${eventInstance.id}" params="[idPhase:eventPhase.id]"><g:img dir="images" file="refill.png" style="width: 20px;"/></g:link>
                </sec:ifAnyGranted>
            </th>
    ...
    <g:each in="${eventInstance.participants.sort {it.name}}" var="participant">
        ...
        <tr>
            <td colspan="3">TOTAL ; ${eventInstance.participants.count {it.id}}</td>
